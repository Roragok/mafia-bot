...
module.exports = (robot) ->
  ...
  playerHelp = () ->
    ...
    response += "* `@mafiabot lynch [target]` - In an active game this will lynch `target`  where target is case sensitve to an alive player in the game.  Optionally you can set `target` to `noylnch` to vote for no lynch.\n"
